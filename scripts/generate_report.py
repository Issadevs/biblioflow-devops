...
def build_report():
    ...
    quality = [["Contrôle", "Résultat"], ["ESLint", f"{lint_errors} erreur / {lint_warnings} avertissement"], ["Prettier", "Format vérifié dans la CI"], ["npm audit", f"{vulnerabilities} vulnérabilité déclarée"], ["Tests et couverture", "Seuils bloquants"], ["Docker Compose", "Configuration valide ; build contrôlé en CI"], ["Test E2E Docker", "Validé localement et en CI" if integration.get("status") == "passed" else "Exécuté par la CI"], ["Dependabot", "npm, GitHub Actions et Docker surveillés"]]
    quality_table = Table(quality, colWidths=[6 * cm, 9 * cm])
    quality_table.setStyle(base_table_style())
    story.extend([quality_table, Spacer(1, 0.5 * cm), Paragraph("Le workflow GitHub Actions exécute deux jobs parallèles : Qualité et tests d'une part, construction Docker et parcours E2E d'autre part. Les permissions sont limitées à la lecture du dépôt et les jobs ont un délai maximal.", styles["BodyFR"]), Paragraph("Mesures complémentaires : validation stricte des entrées, SQL paramétré, stock non négatif garanti par SQL, limite de corps JSON, en-têtes Helmet, politique CSP Nginx et messages d'erreur internes masqués.", styles["BodyFR"]), PageBreak()])
    ...
    endpoints = [["Méthode", "Route", "Usage"], ["GET", "/health (:3001/:3002)", "Santé des API"], ["GET", "/api/books", "Catalogue"], ["POST", "/api/books", "Nouveau livre"], ["PATCH", "/api/books/:id/stock", "Stock atomique"], ["GET", "/api/loans", "Emprunts"], ["POST", "/api/loans", "Nouvel emprunt"], ["POST", "/api/loans/:id/return", "Retour"]]
